          ...
          {            
            questionData.questions[questionIndex+1].options.map((option, index) => (
              <div 
                ...
                className={`${
                  questionIndex === 0 ? 'bg-[#BEE351] shadow-[0px_4px_0px_1px_#90B62A]' :
                  questionIndex === 1 ? 'bg-[#DD3E3E] shadow-[0px_4px_0px_1px_#8D4509]' :
                  'bg-[#89BCFF] shadow-[0px_4px_0px_1px_#1098EC]'
                } w-full rounded-full text-white py-[16px] text-sm flex justify-center items-center font-medium cursor-pointer hover:translate-y-0.5 transition`}
